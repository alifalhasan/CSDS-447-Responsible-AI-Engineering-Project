...
class ImageGenerator:
    ...
    def __init__(self, model_name: str = "stable-diffusion-v1-5", device: str = "auto", 
    ...
    def _load_sdxl(self):
        ...
        if self.use_refiner:
            ...
            try:
                ...
                logger.warning(f"Failed to load refiner: {e}. Continuing without refiner.")
    ...
    def _load_dalle(self):
        ...
        logger.warning("DALL-E integration not implemented yet. Using Stable Diffusion instead.")
        self._load_stable_diffusion()
    
    def generate_image(self, prompt: str, num_images: int = 1, 
                      guidance_scale: float = 7.5, num_inference_steps: int = 20,
                      seed: int = None, negative_prompt: Optional[str] = None,
                      height: Optional[int] = None, width: Optional[int] = None) -> List[Image.Image]:
        """
    ...
    def generate_batch(self, prompts: List[str], output_dir: str, 
                      num_images_per_prompt: int = 5, **kwargs) -> Dict[str, List[str]]:
        """
        ...
        for i, prompt in enumerate(prompts):
            logger.info(f"Generating images for prompt {i+1}/{len(prompts)}: {prompt[:50]}...")
...
def main():
    ...
    logger.info(f"Generated {sum(len(paths) for paths in results.values())} total images")
