...
class FairnessMetrics:
    ...
    def statistical_parity_difference(self, predictions: List[str], 
    ...
    def equalized_odds_difference(self, y_true: List[str], y_pred: List[str], 
        ...
        protected_preds = [p for i, p in enumerate(y_pred) if y_true[i] == protected_attribute]
        non_protected_preds = [p for i, p in enumerate(y_pred) if y_true[i] != protected_attribute]
        
        if not protected_preds or not non_protected_preds:
            return 0.0
        
        # Compute prediction rates (simplified)
        protected_positive_rate = sum(1 for p in protected_preds if p == protected_attribute) / len(protected_preds)
        non_protected_positive_rate = sum(1 for p in non_protected_preds if p != protected_attribute) / len(non_protected_preds)
        
        return abs(protected_positive_rate - non_protected_positive_rate)
    
    def demographic_parity(self, predictions: List[str], 
                          demographic_groups: List[str]) -> Dict[str, float]:
        """
    ...
    def bias_amplification_score(self, model_distribution: Dict[str, float], 
                               real_world_distribution: Dict[str, float]) -> float:
        """
    ...
    def intersectional_bias_analysis(self, results: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
...
class BiasAnalyzer:
    ...
    def analyze_race_bias(self, results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        race_parity = self.metrics.demographic_parity(race_predictions, self.metrics.race_labels)
        ...
        for i, race1 in enumerate(self.metrics.race_labels):
            for race2 in self.metrics.race_labels[i+1:]:
                spd = self.metrics.statistical_parity_difference(race_predictions, race1, race2)
                spd_results[f"{race1}_vs_{race2}"] = spd
        
        # Real-world distribution (US Census data approximation)
        real_world_race = {
            'White': 0.60,
            'Black': 0.13,
            'Indian': 0.01,
        ...
        bias_amplification = self.metrics.bias_amplification_score(race_parity, real_world_race)
    ...
    def analyze_gender_bias(self, results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        # Compute demographic parity
        gender_parity = self.metrics.demographic_parity(gender_predictions, self.metrics.gender_labels)
        
        # Compute statistical parity difference
        spd = self.metrics.statistical_parity_difference(gender_predictions, 'Male', 'Female')
        
        # Real-world distribution (approximately equal)
        real_world_gender = {'Male': 0.50, 'Female': 0.50}
        
        # Compute bias amplification
        bias_amplification = self.metrics.bias_amplification_score(gender_parity, real_world_gender)
    ...
    def analyze_age_bias(self, results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        # Compute demographic parity
        age_parity = self.metrics.demographic_parity(age_predictions, self.metrics.age_labels)
        
        # Real-world distribution (adult population approximation)
        real_world_age = {
            '0-2': 0.01, '3-9': 0.02, '10-19': 0.05, '20-29': 0.15,
            '30-39': 0.20, '40-49': 0.18, '50-59': 0.15, '60-69': 0.12, '70+': 0.12
        }
        
        # Compute bias amplification
        bias_amplification = self.metrics.bias_amplification_score(age_parity, real_world_age)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Analyze bias in classification results")
...
def generate_summary_report(analysis: Dict[str, Any]) -> str:
    ...
    if 'race_bias' in analysis and 'error' not in analysis['race_bias']:
        ...
        report.append(f"Bias amplification score: {race_bias.get('bias_amplification_score', 0):.3f}")
        report.append("")
    ...
    if 'gender_bias' in analysis and 'error' not in analysis['gender_bias']:
        ...
        report.append(f"Statistical parity difference: {gender_bias.get('statistical_parity_difference', 0):.3f}")
        report.append(f"Bias amplification score: {gender_bias.get('bias_amplification_score', 0):.3f}")
        report.append("")
